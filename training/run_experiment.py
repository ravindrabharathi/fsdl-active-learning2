...
def _setup_parser():
    ...
    parser.add_argument(
        "--early_stopping_patience", type=int, default=5, help="No. of epochs without improvement before stopping early"
    )
    ...
    parser.add_argument(
        "--sampling_method",
        type=str,
        choices=BASIC_SAMPLING_METHODS + MC_SAMPLING_METHODS + MB_SAMPLING_METHODS + ATL_SAMPLING_METHODS,
        default="random",
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="No. of active learning iterations (-1 to iterate until pool is exhausted)",
    )
    parser.add_argument(
        "--al_samples_per_iter", type=int, default=2000, help="No. of samples to query per active learning iteration"
    )
    parser.add_argument(
        "--al_continue_training",
        action="store_true",
        help="Whether to continue training after sampling from pool (instead of training from scratch each time",
    )
...
def _initialize_trainer(model_class, lit_model_class, data, args, logger, al_iteration):
    ...
    early_stopping_callback = pl.callbacks.EarlyStopping(
        monitor="val_f1", mode="max", patience=args.early_stopping_patience
    )
...
def _prepare_dataloaders(new_data, new_labels, data, args, val_split=0.2):
    ...
    new_train_dataloader = torch.utils.data.DataLoader(
        new_train_ds, shuffle=True, batch_size=args.batch_size, num_workers=args.num_workers
    )
    new_val_dataloader = torch.utils.data.DataLoader(
        new_val_ds, shuffle=False, batch_size=args.batch_size, num_workers=args.num_workers
    )
...
def main():
    ...
    if args.data_class == "DeepweedsDataModule":
        warnings.warn(
            "DeepweedsDataModule currently only implemented for RGBResnetClassifier with 9 classes, setting args parameters 'n_classes' and 'model_class' manually"
        )
        args.n_classes = 9
        args.model_class = "RGBResnetClassifier"
    elif args.data_class == "CassavaDataModule":
        warnings.warn(
            "CassavaDataModule currently only implemented for RGBResnetClassifier with 5 classes, setting args parameters 'n_classes' and 'model_class' manually"
        )
        args.n_classes = 5
        args.model_class = "RGBResnetClassifier"
    ...
    if sampling_method not in ATL_SAMPLING_METHODS:
    ...
    while unlabelled_data_size > 0 and (args.al_iter < 0 or al_iteration < args.al_iter):
        ...
        if sampling_method in MC_SAMPLING_METHODS:
            ...
            new_indices = _finetune_and_sample(
                lit_model, data, new_train_dataloader, new_val_dataloader, sample_size, args
            )
