...
class ResnetClassifier(nn.Module):
    ...
    def __init__(self, data_config: Dict[str, Any] = None, args: argparse.Namespace = None) -> None:
        ...
        if rgb is True:
            self.preprocess = tt.Compose(
                [tt.Resize(224), tt.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])]
            )
        ...
        if rgb is False:
            ...
            new_layer.weight[
                :, : existing_layer.in_channels, :, :
            ] = existing_layer.weight.clone()  # copying the weights from the old to the new layer

            copy_weights = 0  # take channel 0 weights to initialize new ones
            for i in range(
                n_channels - existing_layer.in_channels
            ):  # copying the weights of the `copy_weights` channel of the old layer to the extra channels of the new layer
                channel = existing_layer.in_channels + i
                new_layer.weight[:, channel : channel + 1, :, :] = existing_layer.weight[
    ...
    def add_to_argparse(parser):
        ...
        parser.add_argument(
            ...
            action="store_true",
            default=False,
            help="Whether to deactivate the training of the ResNet backbone model",
        )
